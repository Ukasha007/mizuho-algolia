...
class AuthenticationService {
  ...
  verifyApiKey(req) {
    ...
    if (req.headers['x-vercel-cron']) {
      this.logger.info('Request authenticated via Vercel Cron');
    ...
    if (authHeader) {
      ...
      if (this.isValidToken(token)) {
        this.logger.info('Request authenticated via Authorization header');
    ...
    if (apiKeyHeader && this.isValidToken(apiKeyHeader)) {
      this.logger.info('Request authenticated via x-api-key header');
