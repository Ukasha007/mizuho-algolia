...
class AlgoliaIndexer {
  constructor() {
    ...
    this.activeSyncs = new Set(); // Set of collectionId or 'static-pages'
  }
  ...
  async performFullSync(options = {}) {
    ...
    this.logger.step('Starting full synchronization to Algolia', {
      region,
      includeStatic,
      includeCMS,
      clearIndex,
    ...
    try {
      ...
      this.logger.info(`Prepared ${allObjects.length} total objects for indexing`);
  ...
  async syncStaticPages(region = null) {
    this.logger.step('Syncing static pages');
    ...
    try {
      ...
      this.logger.success(`Prepared ${transformedPages.length} static pages for indexing`);
  ...
  async syncCMSCollections(region = null) {
    this.logger.step('Syncing CMS collections');
    ...
    try {
      ...
      this.logger.success(`Prepared ${transformedItems.length} CMS items for indexing`);
  ...
  async syncSpecificCollection(collectionSlug, options = {}) {
    ...
    this.logger.step(`Syncing specific collection: ${collectionSlug}`);
    ...
    try {
      ...
      if (!items || items.length === 0) {
        this.logger.warn(`No items found in collection: ${collectionSlug}`);
        return {
          success: true,
          collectionSlug,
          indexed: 0,
      ...
      this.logger.info(`Fetched ${items.length} items from collection ${collectionSlug}`);

      // Deletion sync - remove items that no longer exist in Webflow
      this.logger.step(`Checking for orphaned items in collection: ${collectionSlug}`);
      try {
        ...
        if (deletionResult.deleted > 0) {
          this.logger.success(`Deleted ${deletionResult.deleted} orphaned items from collection ${collectionSlug}`);
        } else {
          this.logger.info(`No orphaned items found in collection ${collectionSlug}`);
        }
      } catch (error) {
        // Log but don't fail the entire sync if deletion fails
        this.logger.error(`Deletion sync failed for collection ${collectionSlug} (continuing with sync)`, {
          error: error.message
        });
      }
      ...
      // Index to Algolia (replaceAllObjects=false means update/add)
      const indexResult = await algoliaClient.indexObjects(preparedObjects, {
        clearIndex: false,
        replaceAllObjects: clearExisting
      });

      this.logger.success(`Synced ${indexResult.indexed} items from collection ${collectionSlug}`);
  ...
  getIndexingStatus() {
    return {
      ...
      stats: this.indexingStats,
      progress: this.isIndexing ? 'in_progress' : 'idle'
    };
  }
